...
import time, os
# import random
import models.EANN.process_twitter as process_data
import copy
...
class Rumor_Data(Dataset):
    def __init__(self, dataset):
        self.post_id = dataset['post_id']
        self.original_post = dataset['original_post']
    ...
    def __getitem__(self, idx):
        return (self.text[idx], self.image[idx], self.mask[idx]), self.label[idx], self.event_label[idx], self.post_id[idx], self.original_post[idx]
...
class ReverseLayerF(Function):
    ...
    def backward(ctx, grad_output):
        # 计算梯度
        grad_input = grad_output * -ctx.lambd
        return grad_input, None  # 返回值数量与 forward 输入一致


def grad_reverse(x, lambd):
    # 调用自定义 Function 的 apply 方法
...
class CNN_Fusion(nn.Module):
    def __init__(self, args, W):
        ...
        # vgg_19 = torchvision.models.vgg19(weights=None)
        model_path = "./vgg19-dcbb9e9d.pth"
        vgg_19 = torchvision.models.vgg19(pretrained=False)
        vgg_19.load_state_dict(torch.load(model_path, weights_only=True))
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels, _, _) in enumerate(train_loader):
        ...
        for i, (validate_data, validate_labels, event_labels, _, _) in enumerate(validate_loader):
    ...
    test_pred = []
    test_true = []
    for i, (test_data, test_labels, event_labels, _, _) in enumerate(test_loader):
...
def parse_arguments(parser):
    ...
    parser.add_argument('--event_num', type=int, default=28, help='')
...
def word2vec(post, word_id_map, W, args):
    ...
    for sentence in post:
        ...
        mask_seq[:len(words)] = 1.0  # 根据分词后的单词长度设置 mask

        for i, word in enumerate(words):
            # 获取单词对应的词索引，如果单词不存在于 map 中，则使用索引 0（默认值）
            sen_embedding.append(word_id_map.get(word, 0))

        # 补齐到指定长度（args.sequence_len）
...
def load_data(args):
    ...
    # print(train[4][0])
    word_vector_path = '../data/twitter/word_embedding.pickle'
